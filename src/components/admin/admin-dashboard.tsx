...
function CardHeader({ title, period, action }: { title: string; period?: string; action?: string }) {
  return <div className="panel-header"><h2>{title}</h2>{period ? <button className="period-button">{period}<ChevronDown size={12}/></button> : action ? <span className="panel-action">{action}</span> : null}</div>;
}
...
function AIUsageCard() { const [usage,setUsage]=useState<{available?:boolean;inputTokens:number;outputTokens:number;estimatedCost:number;requests:number}|null>(null); useEffect(()=>{fetch("/api/admin/usage").then(r=>r.ok?r.json():null).then(setUsage).catch(()=>null)},[]); if(!usage||usage.available===false) return <section className="panel ai-usage"><CardHeader title="AI Usage (Tokens)"/><EmptyState icon={BrainCircuit} title="Usage tracking unavailable" copy="Apply the AI usage migration to start recording token and cost metrics."/></section>; const formatTokens=(value:number)=>value>=1000000?`${(value/1000000).toFixed(2)}M`:value>=1000?`${(value/1000).toFixed(1)}K`:String(value); const total=usage.inputTokens+usage.outputTokens; return <section className="panel ai-usage"><CardHeader title="AI Usage (Tokens)" period="All time"/><div className="ai-usage-body"><div><strong>{formatTokens(total)}</strong><span>Total Tokens</span></div><div className="ai-usage-grid"><p><b>{formatTokens(usage.inputTokens)}</b><span>Input Tokens</span></p><p><b>{formatTokens(usage.outputTokens)}</b><span>Output Tokens</span></p><p><b>${usage.estimatedCost.toFixed(2)}</b><span>Estimated Cost</span></p><p><b>{usage.requests.toLocaleString()}</b><span>AI Requests</span></p></div></div></section>; }
