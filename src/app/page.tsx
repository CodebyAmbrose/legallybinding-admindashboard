...
export default function AdminDashboard() {
  ...
  return <div className="admin-shell">
    <aside className={`sidebar ${mobileOpen ? "open" : ""}`}>
      ...
      <div className="system-status"><span className="status-dot"/><div><b>All systems operational</b><small>System status</small></div></div>
    </aside>
    {mobileOpen && <button className="backdrop" onClick={() => setMobileOpen(false)} aria-label="Close menu"/>}

    <main className="main">
      <header className="topbar"><button className="menu-btn" onClick={() => setMobileOpen(true)} aria-label="Open menu"><Menu size={21}/></button><div className="crumb">Admin / <b>{active}</b></div><div className="top-actions"><button className="icon-btn" aria-label="Notifications"><Bell size={18}/><i/></button><div className="admin-user"><span className="avatar">AR</span><div><b>Ambrose</b><small>Administrator</small></div><ChevronDown size={15}/></div></div></header>
      <div className="content">
        <div className="page-heading"><div><p className="eyebrow">OVERVIEW</p><h1>Admin overview</h1><p className="subtitle">Monitor your legal workspace, users, documents and AI activity.</p></div><div className="heading-actions"><label className="search global-search"><Search size={17}/><input placeholder="Search anything..."/></label><button className="range"><Clock3 size={16}/>Last 30 days<ChevronDown size={14}/></button><button className="export"><Download size={16}/>Export</button></div></div>

        <section className="metrics"><Metric icon={Users} value="2,847" label="Total users" tone="blue"/><Metric icon={Activity} value="1,932" label="Active users" tone="purple"/><Metric icon={FileText} value="12,406" label="Documents uploaded" tone="green"/><Metric icon={CheckCircle2} value="10,892" label="Documents processed" tone="orange"/><Metric icon={Sparkles} value="8,641" label="AI analyses completed" tone="pink"/><Metric icon={Database} value="48.6 GB" label="Storage used" tone="cyan"/></section>
